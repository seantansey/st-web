...
import { type Position, positions } from './config.ts'

const backgroundNodes = computed<Position[]>(() => {
  return positions
})
</script>
...
<template>
  <section class="about-background">
    <h2>Background</h2>
    <div v-for="(node, i) in backgroundNodes" class="node" :key="i">
      ...
      <div class="node-text">
        <h3>
          {{ node.title }}<span class="at-symbol">@</span
